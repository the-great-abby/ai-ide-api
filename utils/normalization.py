        ...
        if (s.startswith("[") and s.endswith("]")):
            try:
                ...
                if isinstance(parsed, list):
                    ...
                    logger.debug(f"clean_examples_field output: {out} (parsed JSON array)")
                    return out
                else:
                    out = [str(parsed)]
                    logger.debug(f"clean_examples_field output: {out} (parsed JSON non-list)")
                    return out
        ...
        if s.startswith("{") and s.endswith("}"):
            ...
            for match in items:
                ...
                if val is not None:
                    ...
                    if val:
                        result.append(val)
            logger.debug(f"clean_examples_field output: {result} (parsed Postgres array)")
            return result
...
def normalize_rule_dict(data):
    ...
    required_fields = [
        "rule_type", "description", "diff", "added_by", "project", "timestamp",
        "categories", "tags", "examples", "applies_to", "applies_to_rationale", "user_story",
        "scope_level", "scope_id", "version", "status", "submitted_by"
    ]
    ...
    if "submitted_by" not in data or data["submitted_by"] is None:
        data["submitted_by"] = ""
    logging.getLogger("examples_normalization").debug(f"[normalize_rule_dict] output: {data}")
    return data 