...
def patch_requests_with_token(monkeypatch, token):
    session = PatchedSession(token)
    monkeypatch.setattr(requests, "get", session.get)
    monkeypatch.setattr(requests, "post", session.post)

def test_valid_args_dry_run(admin_token, monkeypatch):
#     mock_get.return_value.json.return_value = []
#     mock_get.return_value.status_code = 200
#     mock_post.return_value.status_code = 200
    os.environ["RUNNING_IN_DOCKER"] = "1"
    patch_requests_with_token(monkeypatch, admin_token)
    # Should not raise SystemExit, just print 'No pending proposals found.'
    auto_feedback.main()

def test_missing_env_vars(admin_token, monkeypatch):
#     mock_get.return_value.json.return_value = []
#     mock_get.return_value.status_code = 200
#     mock_post.return_value.status_code = 200
    os.environ["RUNNING_IN_DOCKER"] = "1"
...
def test_invalid_input_file(admin_token, monkeypatch):
#     mock_get.return_value.json.return_value = []
#     mock_get.return_value.status_code = 200
#     mock_post.return_value.status_code = 200
    os.environ["RUNNING_IN_DOCKER"] = "1"
...
def test_valid_args_dry_run_cli(tmp_path, admin_token, monkeypatch):
    os.environ["RUNNING_IN_DOCKER"] = "1"
...
# @mock.patch('misc_scripts.auto_feedback.subprocess.run')
def test_missing_env_vars_cli(tmp_path, admin_token, monkeypatch):
    os.environ["RUNNING_IN_DOCKER"] = "1"
    input_file = tmp_path / 'dummy_input.txt'
    input_file.write_text('dummy')
    sys.argv = ['auto_feedback.py', str(input_file), '--dry-run']
...
# @mock.patch('misc_scripts.auto_feedback.subprocess.run')
def test_invalid_input_file_cli(tmp_path, admin_token, monkeypatch):
    os.environ["RUNNING_IN_DOCKER"] = "1"
