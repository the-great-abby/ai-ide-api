...
def test_user_story_completeness_missing(tmp_path):
    ...
    with patch("builtins.open", mock_open(read_data=makefile_content)) as m_open, patch(
        "glob.glob", side_effect=lambda pat: [] if "Makefile.ai-*" in pat else []
    ), patch("pathlib.Path.glob", return_value=[]), patch(
        "os.path.exists", return_value=True
    ), patch(
        "scripts.user_story_completeness_checker.USER_STORY_DIR", str(user_story_dir)
    ), patch(
        "scripts.user_story_completeness_checker.API_FILES", ["api_file.py"]
    ), patch(
        "scripts.user_story_completeness_checker.MAKEFILES", ["Makefile"]
    ), patch(
        "scripts.user_story_completeness_checker.parse_api_endpoints",
        return_value={"/foo", "/bar"},
    ), patch(
        "scripts.user_story_completeness_checker.parse_makefile_targets",
        return_value={"target1", "target2"},
    ), patch.object(
        user_story_completeness_checker, "logger"
    ) as mock_logger:
