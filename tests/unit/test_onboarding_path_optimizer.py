...
def test_onboarding_path_optimizer(tmp_path):
    ...
    with patch(
        "scripts.onboarding_path_optimizer.read_files",
        side_effect=[docs_content, feedback_content],
    ), patch(
        "scripts.onboarding_path_optimizer.call_ollama", return_value=suggestions
    ), patch(
        "builtins.open", mock_open()
    ) as m_open, patch.object(
        onboarding_path_optimizer, "logger"
