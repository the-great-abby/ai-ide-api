...
def skip_if_no_docker():
    if not RUN_DOCKER_SCRIPT_TESTS:
        pytest.skip(
            "Script-based memory graph tests require Docker on host. Set RUN_DOCKER_SCRIPT_TESTS=1 to enable."
        )
...
def test_memory_add_and_list_node_script():
    ...
    list_result = subprocess.run(
        ["make", "-f", "Makefile.ai", "ai-memory-list-nodes"], capture_output=True
    )
...
def test_memory_add_and_list_edge_script():
    ...
    list_edges = subprocess.run(
        ["make", "-f", "Makefile.ai", "ai-memory-list-edges"], capture_output=True
    )
...
def test_memory_traverse_multi_hop_script():
    ...
    traverse = subprocess.run(
        ["make", "-f", "Makefile.ai", "ai-memory-traverse-multi-hop", f"NODE_ID={idA}"],
        capture_output=True,
    )
    assert traverse.returncode == 0
    out = traverse.stdout
...
def test_memory_export_dot_script():
    ...
    # Export DOT
    dot = subprocess.run(
        ["make", "-f", "Makefile.ai", "ai-memory-export-dot"], capture_output=True
    )
