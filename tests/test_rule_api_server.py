...
@pytest.mark.unit
def test_propose_rule_change(client, admin_headers):
...
def test_list_pending_rule_changes(client, admin_headers):
    response = client.get("/list-pending-rule-changes", headers=admin_headers)
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)


def test_approve_and_reject_rule_change(client, admin_headers):
...
def test_reject_rule_change(client, admin_headers):
    # First propose a rule change
    prop_response = client.post(
        "/propose-rule-change",
        json={
...
def test_list_rules(client, admin_headers):
    response = client.get("/list-rules", headers=admin_headers)
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)


def test_rules_mdc_endpoint(client, admin_headers):
    response = client.get("/rules.mdc", headers=admin_headers)
    assert response.status_code == 200
    assert response.headers["content-type"] == "text/markdown"


def test_review_code_files_endpoint(client, admin_headers):
...
def test_review_code_snippet_endpoint(client, admin_headers):
    response = client.post(
        "/review-code-snippet",
        json={
            "filename": "test.py",
...
def test_rule_versioning_and_history(client, admin_headers):
    # First propose a rule change
    prop_response = client.post(
        "/propose-rule-change",
        json={
    ...
    # Get rule history
    history_response = client.get(f"/rule-history/{prop_id}", headers=admin_headers)
...
def test_bug_report_endpoint(client, admin_headers):
    response = client.post(
        "/report-bug",
        json={
    ...
    data = response.json()
    assert "id" in data
    assert data["description"] == "Test bug report"


def test_suggest_enhancement_and_list(client, admin_headers):
    # Suggest enhancement
    ...
    assert enh_data["description"] == "Test enhancement"

    # List enhancements
    list_response = client.get("/list-enhancements", headers=admin_headers)
...
def test_enhancement_to_proposal_and_reject(client, admin_headers):
    # Suggest enhancement
    enh_response = client.post(
        "/suggest-enhancement",
        json={
...
def test_reject_enhancement(client, admin_headers):
    # Suggest enhancement
    enh_response = client.post(
        "/suggest-enhancement",
        json={
...
def test_proposal_to_enhancement(client, admin_headers):
    # First propose a rule change
    prop_response = client.post(
        "/propose-rule-change",
        json={
...
def test_accept_and_complete_enhancement(client, admin_headers):
    # Suggest enhancement
    enh_response = client.post(
        "/suggest-enhancement",
        json={
...
def test_changelog_markdown_endpoint(client, admin_headers):
    response = client.get("/changelog.md", headers=admin_headers)
    assert response.status_code == 200
    assert response.headers["content-type"] == "text/markdown"
    assert "# Changelog" in response.text


def test_changelog_json_endpoint(client, admin_headers):
...
def test_rules_multi_category_filter(client, admin_headers):
    ...
    response = client.get("/list-rules?categories=automation", headers=admin_headers)
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)
    assert len(data) > 0
...
def test_memory_graph_node_crud(client, admin_headers):
    # Create a node
...
def test_memory_graph_edge_crud_and_traversal(client, admin_headers):
    # Create two nodes
...
def test_memory_graph_vector_search(client, admin_headers):
...
def test_rule_proposal_categories_field(client, admin_headers):
    response = client.post(
        "/propose-rule-change",
        json={
...
def test_patch_onboarding_progress(client, admin_headers):
    response = client.patch(
        "/onboarding/progress",
        json={"project_id": "test-project", "step": "completed", "status": "success"},
        headers=admin_headers,
    )
    assert response.status_code == 200
    data = response.json()
...
def test_review_code_files_llm_endpoint(client, admin_headers):
    # Create a temporary test file
    with tempfile.NamedTemporaryFile(suffix=".py", delete=False) as f:
        f.write(b"def test_function():\n    print('test')\n")
        test_file = f.name
...
def test_onboarding_init_idempotency(client, admin_headers):
    payload = {
    ...
    assert data1["project_id"] != data3["project_id"]
