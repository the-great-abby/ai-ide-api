...
def test_backup_nuke_restore_cycle(admin_headers_fixture):
    ...
    if BACKUP_RESTORE_PHASE == "setup":
        ...
        r = requests.post(
            f"{API_URL}/propose-rule-change", json=rule_payload, headers=headers
        )
        assert r.status_code == 200
        proposal_id = r.json()["id"]
        r2 = requests.put(
            f"{API_URL}/rule-changes/{proposal_id}/approve", headers=headers
        )
        ...
        r3 = requests.post(
            f"{API_URL}/memory/nodes", json=node_payload, headers=headers
        )
        ...
        r4 = requests.post(
            f"{API_URL}/memory/nodes", json=node2_payload, headers=headers
        )
        ...
        r5 = requests.post(
            f"{API_URL}/memory/edges", json=edge_payload, headers=headers
        )
        ...
        r6 = requests.post(
            f"{API_URL}/propose-rule-change", json=proposal_payload, headers=headers
        )
        ...
        r7 = requests.post(
            f"{API_URL}/propose-rule-change", json=reject_payload, headers=headers
        )
        assert r7.status_code == 200
        reject_proposal_id = r7.json()["id"]
        r8 = requests.post(
            f"{API_URL}/reject-rule-change/{reject_proposal_id}", headers=headers
        )
        ...
        r9 = requests.post(
            f"{API_URL}/suggest-enhancement", json=enhancement_payload, headers=headers
        )
        assert r9.status_code == 200
        enhancement_id = r9.json()["id"]
        r10 = requests.post(
            f"{API_URL}/accept-enhancement/{enhancement_id}", headers=headers
        )
        assert r10.status_code == 200
        r11 = requests.post(
            f"{API_URL}/complete-enhancement/{enhancement_id}", headers=headers
        )
        ...
        r12 = requests.post(f"{API_URL}/bug-report", json=bug_payload, headers=headers)
        ...
        r13 = requests.post(
            f"{API_URL}/propose-rule-change", json=update_payload, headers=headers
        )
        assert r13.status_code == 200
        update_proposal_id = r13.json()["id"]
        r14 = requests.put(
            f"{API_URL}/rule-changes/{update_proposal_id}/approve", headers=headers
        )
        assert r14.status_code == 200
        del_edge = requests.delete(
            f"{API_URL}/memory/edges",
            params={
                "from_id": node_id,
                "to_id": node2_id,
                "relation_type": "backup_edge",
        ...
        r15 = requests.post(
            f"{API_URL}/memory/nodes", json=node3_payload, headers=headers
        )
        assert r15.status_code == 200
        node3_id = r15.json()["id"]
        print(
            "[INFO] Please run the backup/restore cycle from the host using scripts/backup_restore_cycle.sh, then re-run this test with BACKUP_RESTORE_PHASE=verify to check data integrity after restore."
        )
        return
        ...
        # Rule (latest version)
        rules = requests.get(f"{API_URL}/rules", headers=headers).json()
        ...
        history = requests.get(
            f"{API_URL}/rules/{rule_id}/history", headers=headers
        ).json()
        ...
        nodes = requests.get(
            f"{API_URL}/memory/nodes",
            params={"namespace": f"backupns-{unique}"},
            headers=headers,
        ).json()
        ...
        edges = requests.get(
            f"{API_URL}/memory/edges", params={"from_id": node_id}, headers=headers
        ).json()
        if any(
            e["to_id"] == node2_id and e["relation_type"] == "backup_edge"
            for e in edges
        ):
        ...
        proposals = requests.get(
            f"{API_URL}/pending-rule-changes", headers=headers
        ).json()
        ...
        # Rejected proposal
        all_rules = requests.get(f"{API_URL}/rules", headers=headers).json()
        rejected = [
            r
            for r in all_rules
            if r.get("status") == "rejected" and unique in r.get("description", "")
        ]
        if not rejected:
            print("[DEBUG] Rejected rules after restore:", all_rules)
        assert rejected
        # Completed enhancement
        enhancements = requests.get(f"{API_URL}/enhancements", headers=headers).json()
        completed = [
            e
            for e in enhancements
            if unique in e["description"] and e["status"] == "completed"
        ]
        if not completed:
            print("[DEBUG] Enhancements after restore:", enhancements)
        assert completed
        # Bug report
        bugs = requests.get(f"{API_URL}/bug-reports", headers=headers).json()
        ...
        # Multiple namespaces
        nodes2 = requests.get(
            f"{API_URL}/memory/nodes", params={"namespace": ns2}, headers=headers
        ).json()
...
def test_backup_and_restore(admin_headers_fixture):
    headers = admin_headers_fixture
