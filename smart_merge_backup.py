...
def main():
    parser = argparse.ArgumentParser(
        description="Smart merge backup script for Postgres."
    )
    ...
    if not livedb:
        ...
        if livedb:
            ...
            log(
                "Missing required environment variable: PGDATABASE and could not auto-detect from filename."
            )
            sys.exit(1)
    else:
    ...
    if not args.dry_run:
        run(
            f'createdb -U {os.environ.get("POSTGRES_USER", pguser)} -h {get_pg_host()} -p {os.environ.get("POSTGRES_PORT", pgport)} {temp_db}'
        )
        run(
            f'psql -U {os.environ.get("POSTGRES_USER", pguser)} -h {get_pg_host()} -p {os.environ.get("POSTGRES_PORT", pgport)} -d {temp_db} -f "{args.backup_sql}"'
        )
    ...
    if not args.dry_run:
        try:
            run(
                f'psql -U {os.environ.get("POSTGRES_USER", pguser)} -h {get_pg_host()} -p {os.environ.get("POSTGRES_PORT", pgport)} -d {livedb} -c "{drop_schema_sql}"',
                check=False,
            )
        except Exception:
            pass  # Ignore errors
    ...
    for cmd in fdw_cmds:
        if not args.dry_run:
            ...
            run(
                f'psql -U {os.environ.get("POSTGRES_USER", pguser)} -h {get_pg_host()} -p {os.environ.get("POSTGRES_PORT", pgport)} -d {livedb} -c "{cmd}"',
                check=False,
            )
        else:
    ...
    for table in safe_tables:
        ...
        if not args.dry_run:
            ...
            rc = run(
                f'psql -U {os.environ.get("POSTGRES_USER", pguser)} -h {get_pg_host()} -p {os.environ.get("POSTGRES_PORT", pgport)} -d {livedb} -c "{import_cmd}"',
                check=False,
            )
            if rc != 0:
    ...
    for table, (columns, upsert_sql) in TABLES.items():
        if table in present_tables:
            ...
            if not table_has_columns(live_conn, table, columns):
                log(
                    f"Skipping upsert for {table}: columns do not match live DB schema."
                )
                continue
            if not foreign_table_exists(temp_conn, "temp_schema", table):
                log(
                    f"Skipping upsert for {table}: foreign table temp_schema.{table} does not exist."
                )
                continue
            ...
            if args.dry_run:
                ...
                run(
                    f'psql -U {os.environ.get("POSTGRES_USER", pguser)} -h {get_pg_host()} -p {os.environ.get("POSTGRES_PORT", pgport)} -d {livedb} -c "{upsert_sql}"'
                )
        else:
    ...
    if not args.dry_run:
        run(
            f'psql -U {os.environ.get("POSTGRES_USER", pguser)} -h {get_pg_host()} -p {os.environ.get("POSTGRES_PORT", pgport)} -d {livedb} -c "{cleanup_sql}"'
        )
        run(
            f'dropdb -U {os.environ.get("POSTGRES_USER", pguser)} -h {get_pg_host()} -p {os.environ.get("POSTGRES_PORT", pgport)} {temp_db}'
        )
    else:
