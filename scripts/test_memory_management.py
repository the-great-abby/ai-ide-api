...
def test_tagging_and_categorization(api_token: str) -> Dict[str, Any]:
    ...
    try:
        ...
        nodes = response.json()
        worker_nodes = [
            node for node in nodes if node.get("namespace") == "worker_commits"
        ]
        ...
        for node in worker_nodes:
            ...
            if missing_categories:
                results["issues"].append(
                    f"Node {node_id} missing categories: {missing_categories}"
                )
...
def test_search_functionality(api_token: str) -> Dict[str, Any]:
    ...
    try:
        ...
        for query in test_queries:
            ...
            # Test RAG search
            rag_payload = {"question": query, "namespace": "worker_commits", "top_k": 5}
...
def test_deduplication(api_token: str) -> Dict[str, Any]:
    ...
    try:
        ...
        response = requests.post(
            "http://api:8000/memory/nodes", json=test_story, headers=headers, timeout=30
        )
        ...
        if response.status_code == 200:
            ...
            results = {
                ...
                "duplicate_attempt_response": response2.text
                if response2.status_code != 200
                else "Success",
                "deduplication_working": response2.status_code
                != 200,  # If it fails, deduplication might be working
            }

            # Clean up test nodes
            try:
                requests.delete(
                    f"http://api:8000/memory/nodes/{node_id}", headers=headers
                )
...
def test_similarity_pruning(api_token: str) -> Dict[str, Any]:
    ...
    try:
        ...
        length_variance = (
            sum((length - avg_length) ** 2 for length in content_lengths)
            / len(content_lengths)
            if content_lengths
            else 0
        )
...
def main():
    ...
    parser.add_argument(
        "--test",
        choices=["all", "tagging", "search", "dedup", "pruning", "cleanup"],
        default="all",
    ...
    for test_name, test_results in results.items():
        ...
        if "error" in test_results:
            ...
            if test_name == "tagging":
                logger.info(
                    f"   - Total worker stories: {test_results.get('total_worker_stories', 0)}"
                )
                ...
                logger.info(
                    f"   - Total queries tested: {len(test_results.get('queries', []))}"
                )
                logger.info(
                    f"   - Total results found: {test_results.get('total_results', 0)}"
                )

                for query_result in test_results.get("queries", []):
                    status = "✅" if query_result.get("success") else "❌"
                    logger.info(
                        f"   {status} '{query_result['query']}': {query_result.get('rag_results', 0)} results"
                    )

            elif test_name == "deduplication":
                logger.info(
                    f"   - Deduplication working: {test_results.get('deduplication_working', False)}"
                )
                logger.info(
                    f"   - Original node: {test_results.get('original_node_id', 'N/A')}"
                )
                ...
                logger.info(
                    f"   - Potential duplicates: {test_results.get('potential_duplicates', 0)}"
                )
                logger.info(
                    f"   - Content length variance: {test_results.get('content_length_stats', {}).get('variance', 0):.2f}"
                )

            elif test_name == "memory_cleanup":
                logger.info(f"   - Total nodes: {test_results.get('total_nodes', 0)}")
                logger.info(
                    f"   - Cleanup issues: {test_results.get('issues_count', 0)}"
                )
