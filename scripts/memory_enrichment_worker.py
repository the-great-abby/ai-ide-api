...
async def retry_with_backoff(func, *args, max_retries=MAX_RETRIES, delay=RETRY_DELAY):
    ...
    raise EnrichmentError(f"Failed after {max_retries} attempts. Last error: {last_error}")
...
async def process_node_batch(nodes: List[MemoryVector], max_tags: int) -> List[Dict[str, Any]]:
...
async def process_batch(batch: List[MemoryVector], job_config: Dict[str, Any]) -> Tuple[int, int]:
...
async def extract_tags_and_categories(nodes: List[MemoryVector]) -> Tuple[List[str], List[str]]:
