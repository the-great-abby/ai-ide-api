...
WORKER_COMMITS = [
    ...
    },
    {
        ...
        "files": ["Makefile.ai-memory", "docs/user_stories/memory_similarity_pruning_worker.md"]
    },
    {
        "hash": "80417b5f",
...
def get_commit_info(commit_hash: str) -> Dict[str, Any]:
    ...
    try:
        # Get commit details
        cmd = ["git", "show", "--no-pager", "--pretty=format:%H|%an|%ad|%s", "--date=short", "--name-only", commit_hash]
...
def create_worker_commit_story(commit_info: Dict[str, Any], commit_description: str) -> Dict[str, Any]:
    ...
    story.append(f"This commit represents a focused change by {commit_info['author']} related to worker functionality.")
    
    # Analyze worker-related files
    worker_files = [f for f in commit_info['files_changed'] if any(keyword in f.lower() for keyword in ['worker', 'memory', 'dockerfile', 'makefile'])]
    if worker_files:
    ...
    story.append(f"This commit touches {len(commit_info['files_changed'])} files, suggesting a ")
    ...
    return {
        ...
        "tags": [
            ...
            "worker-system",
            "memory-workers"
        ] + summary_result["categories"] + summary_result["tags"]
    }
...
def main():
    parser = argparse.ArgumentParser(description="Create individual detailed stories for worker-related commits")
