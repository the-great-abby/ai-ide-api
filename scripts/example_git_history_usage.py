...
def example_time_period_analysis():
    ...
    analyzed_commits = analyze_commit_range(
        commits, include_diff=True, summarize=False, batch_size=3
    )
...
def example_custom_analysis():
    ...
    commits = get_commit_list(since="2 weeks ago", until="1 day ago", max_commits=20)
    ...
    analyzed_commits = analyze_commit_range(
        python_commits, include_diff=True, summarize=False, batch_size=5
    )
...
def main():
    ...
    try:
        ...
        print(
            "  python3 scripts/git_history_analyzer.py --since '1 week ago' --max-commits 10"
        )
        print("\nOr use the Makefile target:")
        print(
            "  make -f Makefile.ai misc-git-history-analyze SINCE='1 week ago' MAX_COMMITS=10"
        )
