...
OLLAMA_URL = os.environ.get(
    "OLLAMA_URL",
    "http://api:8000/summarize-git-diff"
...
def main():
    parser = argparse.ArgumentParser(
        description="Summarize a git diff using LLM (Ollama functions)."
    )
    parser.add_argument(
        "--diff-file", help="Path to a git diff file. If omitted, read from stdin."
    )
    parser.add_argument(
        "--concise", action="store_true", help="Use a concise summary prompt."
    )
    ...
    for i, chunk in enumerate(chunks):
        print(
            f"[INFO] Summarizing chunk {i+1}/{len(chunks)} (length: {len(chunk)} chars)...",
            file=sys.stderr,
        )
