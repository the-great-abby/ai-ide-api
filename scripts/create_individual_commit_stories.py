...
from scripts.git_history_analyzer import (
    get_commit_list, 
    analyze_commit, 
    GitCommit
)
...
def get_commit_list_fixed(since: str = None, until: str = None, 
                         max_commits: int = 50, author: str = None) -> List[GitCommit]:
    ...
    try:
        ...
        result = subprocess.run(cmd, capture_output=True, text=True, check=True, cwd="/code")
...
def analyze_commit_fixed(commit: GitCommit, include_diff: bool = True, summarize: bool = True) -> GitCommit:
...
def summarize_diff(diff: str, commit_message: str, author: str, concise: bool = True) -> Dict[str, Any]:
    ...
    try:
        ...
        response = requests.post("http://api:8000/summarize-git-diff", json=payload, timeout=120)
...
def create_individual_commit_story(commit: GitCommit, memory_namespace: str = "individual_commits") -> Dict[str, Any]:
    ...
    story.append(f"This commit represents a focused change by {analyzed_commit.author}.")
    story.append(f"It touches {len(analyzed_commit.files_changed)} files, suggesting a ")
    ...
    return {
        ...
        ] + analyzed_commit.categories + analyzed_commit.tags
    }
...
def main():
    parser = argparse.ArgumentParser(description="Create individual detailed stories for each commit")
    ...
    parser.add_argument("--max-commits", type=int, default=10, help="Maximum number of commits to analyze")
    parser.add_argument("--author", help="Filter by author")
    parser.add_argument("--memory-namespace", default="individual_commits", help="Memory namespace for stories")
    ...
    for i, commit in enumerate(commits, 1):
        logger.info(f"Processing commit {i}/{len(commits)}: {commit.commit_hash[:8]} by {commit.author}")
        ...
        try:
            ...
            if args.dry_run:
                logger.info(f"DRY RUN: Would create story for commit {commit.commit_hash[:8]}")
            ...
            if memory_id:
                logger.info(f"✅ Created story for commit {commit.commit_hash[:8]} (ID: {memory_id})")
                ...
                logger.error(f"❌ Failed to store story for commit {commit.commit_hash[:8]}")
    ...
    if created_stories:
        ...
        for story in created_stories:
            logger.info(f"  • {story['commit_hash']} by {story['author']} ({story['files']} files) → {story['memory_id']}")
