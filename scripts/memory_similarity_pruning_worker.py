...
async def find_similar_nodes(nodes: List[MemoryVector]) -> List[SimilarityGroup]:
    ...
    for namespace, ns_nodes in by_namespace.items():
        ...
        for i, node1 in enumerate(ns_nodes):
            ...
            if any(node1 in [s[0] for s in group.similar] for group in similarity_groups):
                continue
            ...
            for node2 in ns_nodes[i+1:]:
                ...
                if any(node2 in [s[0] for s in group.similar] for group in similarity_groups):
                    continue
...
async def process_similarity_pruning_job(job_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        VECTOR_SIMILARITY_THRESHOLD = job_config.get("vector_similarity_threshold", VECTOR_SIMILARITY_THRESHOLD)
        CONTENT_SIMILARITY_THRESHOLD = job_config.get("content_similarity_threshold", CONTENT_SIMILARITY_THRESHOLD)
        TAG_OVERLAP_THRESHOLD = job_config.get("tag_overlap_threshold", TAG_OVERLAP_THRESHOLD)
        ...
        async with MemorySessionLocal() as session:
            ...
            if job_config.get("scope") == "new":
                ...
                subquery = session.query(MemoryEdge.from_id)\
