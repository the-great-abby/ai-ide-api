...
def initialize_onboarding(api_url: str, project_name: str, onboarding_path: str) -> bool:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"❌ Onboarding initialization failed: {response.status_code} - {response.text}")
            return False
...
def test_memory_api(api_url: str, token: str) -> bool:
    ...
    try:
        ...
        payload = {
            ...
            "meta": json.dumps({"tags": ["onboarding", "test"], "source": "external_script"})
        }
        ...
        if response.status_code == 200:
            result = response.json()
            print(f"✅ Memory API test successful - created node: {result.get('id', 'unknown')}")
            return True
        else:
