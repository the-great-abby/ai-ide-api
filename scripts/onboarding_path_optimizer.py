...
ONBOARDING_DOCS = sorted(glob.glob("docs/onboarding/*.md") + ["ONBOARDING.md"])  # Add main onboarding doc
...
def main():
    ...
    parser.add_argument("--dry-run", action="store_true", help="Preview suggestions only")
    args = parser.parse_args()

    docs = read_files(ONBOARDING_DOCS)
    feedback = read_files(FEEDBACK_FILES)
    logger.info(f"Loaded {len(ONBOARDING_DOCS)} onboarding docs and {len(FEEDBACK_FILES)} feedback files.")

    prompt = LLM_PROMPT.format(docs=docs[:4000], feedback=feedback[:2000])  # Truncate for prompt size
    logger.info("Calling LLM to analyze onboarding docs and feedback...")
    suggestions = call_ollama(prompt)
    if suggestions:
        logger.info("[LLM SUGGESTIONS]\n" + suggestions)
        if not args.dry_run:
            with open("docs/onboarding/onboarding_optimization_suggestions.md", "w") as f:
                f.write("# Onboarding Optimization Suggestions\n\n")
                f.write(suggestions)
            logger.info("[CREATED] docs/onboarding/onboarding_optimization_suggestions.md")
    else:
