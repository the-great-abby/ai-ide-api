...
def main():
    ...
    parser.add_argument(
        ...
        help="Days since last update to consider stale",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Preview flagged entries without taking action",
    )
    ...
    logger.info(
        f"Flagged {len(stale_mem)} stale memory nodes and {len(deprecated_mem)} deprecated/obsolete memory nodes."
    )
    for n in stale_mem:
        logger.info(
            f"[STALE MEMORY] id={n.id} ns={n.namespace} created={n.created_at} meta={n.meta}"
        )
    for n in deprecated_mem:
        logger.info(
            f"[DEPRECATED MEMORY] id={n.id} ns={n.namespace} created={n.created_at} meta={n.meta}"
        )
    mem_session.close()
    ...
    deprecated_rules = [
        r for r in rules if is_deprecated(parse_meta(getattr(r, "meta", None)))
    ]
    logger.info(
        f"Flagged {len(stale_rules)} stale rules and {len(deprecated_rules)} deprecated/obsolete rules."
    )
    for r in stale_rules:
        logger.info(
            f"[STALE RULE] id={r.id} desc={r.description[:80]}... timestamp={r.timestamp}"
        )
    ...
    logger.info(
        "Dry run: no issues opened or notifications sent. Review logs for flagged entries."
    )
