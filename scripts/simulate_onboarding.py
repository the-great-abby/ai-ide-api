...
def simulate_onboarding(project_name, path):
    ...
    resp = requests.post(
        f"{API_URL}/onboarding/init", json={"project_name": project_name, "path": path}
    )
    ...
    if progress:
        ...
        resp = requests.patch(
            f"{API_URL}/onboarding/progress/{step_id}", json={"completed": True}
        )
        print(
            f"Mark complete for step '{progress[0]['instruction']}':",
            resp.status_code,
            resp.json(),
        )
    ...
    print(
        "Onboarding docs (first 200 chars):", resp.status_code, resp.text[:200], "..."
    )
    resp = requests.get(f"{API_URL}/onboarding/user_story/{path}")
    print(
        f"User story for path '{path}' (first 200 chars):",
        resp.status_code,
        resp.text[:200],
        "...",
    )
...
def main():
    if len(sys.argv) < 3:
        print(
            "Usage: python scripts/simulate_onboarding.py <project_name> <onboarding_path>"
        )
        print(
            "Example: python scripts/simulate_onboarding.py test_onboarding external_project"
        )
        sys.exit(1)
