...
def main():
    ...
    for node in nodes:
        if node.content and len(node.content) > LONG_CONTENT_THRESHOLD:
            prompt = f"Summarize the following memory node content in 1-2 sentences, preserving all key details:\n\n{node.content}"
            summary = call_ollama(prompt)
            if summary and summary.strip() != node.content.strip():
                logger.info(
                    f"[SUMMARY] id={node.id} ns={node.namespace}\n  Original: {node.content[:100]}...\n  Summary: {summary[:100]}..."
                )
                if args.apply:
    ...
    for node in nodes:
        ...
        if is_ambiguous(node.content, meta):
            prompt = f"Rewrite the following memory node content to be clear and unambiguous. If information is missing, note it explicitly.\n\n{node.content}"
            clarified = call_ollama(prompt)
            if clarified and clarified.strip() != node.content.strip():
                logger.info(
                    f"[CLARIFY] id={node.id} ns={node.namespace}\n  Original: {node.content[:100]}...\n  Clarified: {clarified[:100]}..."
                )
                if args.apply:
                    meta = parse_meta(node.meta)
                    meta["original_content"] = node.content
    ...
    for n1, n2, ratio in merge_suggestions:
        logger.info(
            f"[MERGE SUGGESTION] ns={n1.namespace} id1={n1.id} id2={n2.id} similarity={ratio:.2f}"
        )
    ...
    if args.apply:
        ...
        logger.info(
            "Dry run: no changes applied. Review logs for proposed refinements."
        )
    session.close()
