...
API_URL = os.environ.get(
    ...
    else "http://localhost:9103/memory/nodes",
)
OLLAMA_URL = os.environ.get(
    "OLLAMA_URL",
    "http://api:8000/suggest-llm-rules"
    if os.environ.get("RUNNING_IN_DOCKER")
    else "http://localhost:9103/suggest-llm-rules",
)
...
def main():
    parser = argparse.ArgumentParser(
        description="Create a memory node from a user story markdown file."
    )
    parser.add_argument(
        "--file", required=True, help="Path to the user story markdown file"
    )
    parser.add_argument(
