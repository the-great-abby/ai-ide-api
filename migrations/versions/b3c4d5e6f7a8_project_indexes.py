...
def upgrade() -> None:
    ...
    if "api_access_tokens" in inspector.get_table_names():
        ...
        if "role" not in columns:
            op.add_column(
                "api_access_tokens",
                sa.Column(
                    "role", sa.String(length=32), nullable=False, server_default="admin"
                ),
            )
            op.execute(
                """
    ...
    if "memory_vectors" in inspector.get_table_names():
        existing_indexes = [
            ix["name"] for ix in inspector.get_indexes("memory_vectors")
        ]
        if "ix_memory_vectors_project_id" not in existing_indexes:
            op.create_index(
