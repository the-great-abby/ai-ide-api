...
def require_api_token(
    ...
    if db_token:
        logger.debug(
            f"[require_api_token] Token role: {db_token.role}, active: {db_token.active}"
        )
    if not db_token:
        logger.warning(f"[require_api_token] Invalid or inactive token: {token}")
        raise HTTPException(status_code=401, detail="Invalid or inactive token")
    logger.info(
        f"[require_api_token] Valid token: {token}, role: {db_token.role}, active: {db_token.active}"
    )
    return db_token
...
def check_namespace_permission(
    ...
    if token.role == "admin":
        logger.debug(
            f"[check_namespace_permission] Admin token, access granted for {namespace}"
        )
        return True
    ...
    if token.project_id:
        ...
        logger.debug(
            f"[check_namespace_permission] Project token {token.project_id}, checking {len(permissions)} permissions for namespace '{namespace}' and permission '{required_permission}'"
        )
        for perm in permissions:
            logger.debug(
                f"[check_namespace_permission] Checking pattern '{perm.namespace}' against '{namespace}'"
            )
            if fnmatch(namespace, perm.namespace):
                logger.debug(
                    f"[check_namespace_permission] MATCH: '{namespace}' matches '{perm.namespace}'"
                )
                return True
        logger.warning(
            f"[check_namespace_permission] No matching permission for {namespace} (project {token.project_id})"
        )
        raise HTTPException(
            status_code=403,
