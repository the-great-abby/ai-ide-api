...
ENABLE_DIFF_SUMMARIZATION = os.environ.get(
    "ENABLE_DIFF_SUMMARIZATION", "true"
).lower() in ("1", "true", "yes")
...
def get_last_processed_commit() -> Optional[str]:
    ...
    resp = requests.get(
        f"{MEMORY_API_URL}/nodes?namespace={DEFAULT_NAMESPACE}", headers=headers
    )
    ...
    for node in nodes:
        ...
        if (
            ...
            and meta.get("type") == HEAD_COMMIT_NODE_META_KEY
        ):
...
def get_git_diff(base_commit: str, head_commit: str = "HEAD") -> str:
    ...
    result = subprocess.run(
        ["git", "diff", base_commit, head_commit], capture_output=True, text=True
    )
...
def call_llm_summarize(diff: str, commit_msg: str, author: str) -> Dict[str, Any]:
    ...
    resp = requests.post(
        f"{OLLAMA_FUNCTIONS_URL}/summarize-git-diff", json=prompt, timeout=120
    )
...
def create_memory_node(content: str, meta: Dict[str, Any]) -> str:
    ...
    try:
        resp = requests.post(f"{MEMORY_API_URL}/nodes", json=payload, headers=headers)
        logger.debug(
            f"[create_memory_node] Response status: {resp.status_code}, text: {resp.text}"
        )
        resp.raise_for_status()
        return resp.json()["id"]
...
async def process_progress_report_job(body: Dict[str, Any]):
    ...
    try:
        ...
        logger.debug(
            f"[process_progress_report_job] Last processed commit: {last_commit}"
        )
        head_commit = "HEAD"
        if not last_commit:
        ...
        # 2. Get git diff
        logger.debug(
            f"[process_progress_report_job] Getting git diff from {last_commit} to {head_commit}..."
        )
        diff = get_git_diff(last_commit, head_commit)
        logger.debug(
            f"[process_progress_report_job] Git diff: {diff[:200]}..."
            if diff
            else "[process_progress_report_job] No diff found."
        )
        if not diff.strip():
        ...
        logger.debug(
            f"[process_progress_report_job] Commit msg: {commit_msg}, Author: {author}"
        )

        if ENABLE_DIFF_SUMMARIZATION:
            logger.debug(
                "[process_progress_report_job] Diff summarization enabled. Calling LLM..."
            )
            # 4. Call LLM
            try:
                ...
                logger.error(
                    f"[process_progress_report_job] Exception during LLM call: {e}"
                )
            ...
            try:
                ...
                if (
                    not summary_text
                    and "summaries" in llm_result
                    and isinstance(llm_result["summaries"], list)
                    and llm_result["summaries"]
                ):
                    summary_text = llm_result["summaries"][0]
                if not summary_text:
                    logger.error(
                        f"[process_progress_report_job] No summary found in LLM result: {llm_result}"
                    )
                    raise KeyError("No summary found in LLM result")
                summary_id = create_memory_node(
                    summary_text,
                    {
                ...
                logger.debug(
                    f"[process_progress_report_job] Created summary node with id: {summary_id}"
                )
            except Exception as e:
                logger.error(
                    f"[process_progress_report_job] Exception during summary node creation: {e}"
                )
                logger.error(traceback.format_exc())
                raise
            ...
            for question in llm_result.get("open_questions", []):
                try:
                    ...
                    logger.debug(
                        f"[process_progress_report_job] Created open question node {q_id} and edge from {summary_id}"
                    )
                except Exception as e:
                    logger.error(
                        f"[process_progress_report_job] Exception during open question node/edge: {e}"
                    )
                    logger.error(traceback.format_exc())
            ...
            for file in llm_result.get("related_files", []):
                try:
                    ...
                    logger.debug(
                        f"[process_progress_report_job] Created related file node {f_id} and edge from {summary_id}"
                    )
                except Exception as e:
                    logger.error(
                        f"[process_progress_report_job] Exception during related file node/edge: {e}"
                    )
                    logger.error(traceback.format_exc())
        else:
            logger.info(
                "Diff summarization is DISABLED. Creating placeholder summary node."
            )
            ...
            try:
                ...
                logger.error(
                    f"[process_progress_report_job] Exception during placeholder node creation: {e}"
                )
                logger.error(traceback.format_exc())
                raise
        ...
        for rel in llm_result.get("proposed_relationships", []):
            ...
            if target:
                try:
                    t_id = create_memory_node(
                        target, {"type": rel.get("type", "related")}
                    )
                    create_memory_edge(summary_id, t_id, rel.get("type", "related"))
                    logger.debug(
                        f"[process_progress_report_job] Created proposed relationship node {t_id} and edge from {summary_id}"
                    )
                except Exception as e:
                    logger.error(
                        f"[process_progress_report_job] Exception during proposed relationship node/edge: {e}"
                    )
                    logger.error(traceback.format_exc())
        # 9. Update last processed commit
        try:
            new_commit = subprocess.run(
                ["git", "rev-parse", head_commit], capture_output=True, text=True
            ).stdout.strip()
            update_last_processed_commit(new_commit)
            logger.info(f"Progress report processed for commit {new_commit}")
        except Exception as e:
            logger.error(
                f"[process_progress_report_job] Exception during last commit update: {e}"
            )
            logger.error(traceback.format_exc())
    except Exception as e:
