...
API_URL = os.environ.get(
    "MEMORY_API_URL",
    "http://api:8000/memory" if os.environ.get("RUNNING_IN_DOCKER") else "http://localhost:9104/memory"
)
...
def traverse_single_hop(node_id: str) -> List[Dict[str, Any]]:
    ...
    headers = get_auth_header()
    response = requests.get(f"{API_URL}/nodes/{node_id}/traverse?hops=1", headers=headers)
    response.raise_for_status()
    return response.json()
...
def traverse_multi_hop(node_id: str, max_hops: int = 3) -> List[Dict[str, Any]]:
    ...
    headers = get_auth_header()
    response = requests.get(f"{API_URL}/nodes/{node_id}/traverse?hops={max_hops}", headers=headers)
    response.raise_for_status()
    return response.json()
...
def traverse_by_relation(node_id: str, relation_type: str) -> List[Dict[str, Any]]:
    ...
    headers = get_auth_header()
    response = requests.get(f"{API_URL}/nodes/{node_id}/traverse?relation_type={relation_type}", headers=headers)
    response.raise_for_status()
    return response.json()
