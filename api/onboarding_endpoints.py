...
def get_onboarding_path(path_name: str):
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to load onboarding paths: {e}"
        )
    ...
    raise HTTPException(
        status_code=404,
        detail=f"Onboarding journey '{path_name}' not found. Valid options are: {', '.join(valid_journeys)}.",
    )
...
def get_onboard_external_py():
    script_path = os.path.join(
        os.path.dirname(__file__), "../scripts/onboard_external_june2025.py"
    )
...
def get_onboard_external_sh():
    script_path = os.path.join(
        os.path.dirname(__file__), "../scripts/onboard_external.sh"
    )
    with open(script_path, "r") as f:
        return f.read()
